...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] [Tooltip("Players Rigid body.")] 
    Rigidbody2D rb; // Players Rigid body

    [SerializeField] [Tooltip("Defines player's movement speed.")] 
    private float movSpeed; // Defines player's movement speed

    [SerializeField] [Tooltip("Defines the force for jump attribute.")] 
    private float jumpForce; // Defines the force for jump attribute

    [SerializeField] [Tooltip("Status if player is grounded.")]
    private bool isGrounded; // Status if player is grounded

    // ----------------------------------I   I---------------------------------- \\

    [Header("Jump / Fall multipliers")]
    [SerializeField] [Tooltip("How fast players fall speed picks up.")] 
    private float fallMultiplier = 2.5f; // How fast players fall speed picks up

    [SerializeField] [Tooltip("Multiplier on how much player jumps on quick input.")]
    private float lowJumpMultiplier = 2f; // Multiplier on how much player jumps on quick input

    // ----------------------------------I   I---------------------------------- \\

    [Header("Ground check")]
    [SerializeField] [Tooltip("Checks grounds position and determines if \"isGrounded\".")]
    private Transform groundCheckPosition; // Checks grounds position and determines if "isGrounded"

    [SerializeField] [Tooltip("Determines the size of the radious to check if grounded.")] 
    private float groundCheckRadius; // Determines the size of the radious to check if grounded

    [SerializeField] [Tooltip("Creates Gizmo on unity to show the check layer.")] 
